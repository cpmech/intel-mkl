...
/*
!  Content:
!      C B L A S _ S T R S M _ B A T C H  Example Program Text ( C Interface )
!******************************************************************************/
...
int main(int argc, char *argv[])
{
      ...
      float           alpha[MAX_GRP_COUNT];
      float           *a, *b;
      float           **a_array, **b_array;
      CBLAS_LAYOUT    layout;
      ...
      printf("\n     C B L A S _ S T R S M _ B A T C H  EXAMPLE PROGRAM\n");
...
/*       Get input data                                        */
      ...
      for (i=0; i<grp_count; ++i) {
          if( GetScalarsS(in_file, &alpha[i]) != 1 ) {
      ...
      for (i=0; i<grp_count; ++i) {
          ...
          for (j=0; j<grp_sizes[i]; ++j) {
              ...
              if( GetArrayS(in_file, &layout, typeA, &ma, &na, a, &lda[i]) != 0 ) {
              ...
              if( GetArrayS(in_file, &layout, GENERAL_MATRIX, &m[i], &n[i], b, &ldb[i]) != 0 ) {
...
/*       Print input data                                      */
      ...
      for (i=0; i<grp_count; ++i) {
          ...
          for (j=0; j<grp_sizes[i]; ++j) {
              ...
              PrintArrayS(&layout, FULLPRINT, typeA, &ma, &na, a_array[idx], &lda[i], "A");
              PrintArrayS(&layout, FULLPRINT, GENERAL_MATRIX, &m[i], &n[i], b_array[idx], &ldb[i], "B");
              idx++;
          }
      }

/*      Call STRSM_BATCH subroutine ( C Interface )                  */
      cblas_strsm_batch(layout, side, uplo, transA, diag, m, n, alpha,
              (const float **) a_array, lda, b_array, ldb, grp_count, grp_sizes);
