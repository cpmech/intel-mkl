...
/*
!  Content:
!      C B L A S _ Z T R S M _ B A T C H  Example Program Text ( C Interface )
!******************************************************************************/
...
int main(int argc, char *argv[])
{
      ...
      MKL_Complex16   alpha[MAX_GRP_COUNT];
      MKL_Complex16   *a, *b;
      MKL_Complex16   **a_array, **b_array;
      CBLAS_LAYOUT    layout;
      ...
      printf("\n     C B L A S _ Z T R S M _ B A T C H  EXAMPLE PROGRAM\n");
...
/*       Get input data                                        */
      ...
      for (i=0; i<grp_count; ++i) {
          if( GetScalarsZ(in_file, &alpha[i]) != 1 ) {
      ...
      for (i=0; i<grp_count; ++i) {
          ...
          for (j=0; j<grp_sizes[i]; ++j) {
              ...
              if( GetArrayZ(in_file, &layout, typeA, &ma, &na, a, &lda[i]) != 0 ) {
              ...
              if( GetArrayZ(in_file, &layout, GENERAL_MATRIX, &m[i], &n[i], b, &ldb[i]) != 0 ) {
...
/*       Print input data                                      */
      ...
      for (i=0; i<grp_count; ++i) {
          ...
          for (j=0; j<grp_sizes[i]; ++j) {
              PrintArrayZ(&layout, FULLPRINT, typeA, &ma, &na, a_array[idx], &lda[i], "A");
              PrintArrayZ(&layout, FULLPRINT, GENERAL_MATRIX, &m[i], &n[i], b_array[idx], &ldb[i], "B");
              idx++;
          }
      }

/*      Call ZTRSM_BATCH subroutine ( C Interface )                  */
      cblas_ztrsm_batch(layout, side, uplo, transA, diag, m, n, alpha,
              (const void **) a_array, lda, (void **) b_array, ldb,
              grp_count, grp_sizes);
