...
/*
!  Content:
!      C B L A S _ G E M M _ S 8 U 8 S 3 2 _ C O M P U T E  
...
int main(int argc, char *argv[])
{
      ...
      printf("\n     C B L A S _ G E M M _ S 8 U 8 S 3 2 _ C O M P U T E  EXAMPLE PROGRAM\n");
...
/*       Get input data                                                                  */
      ...
      a = (void *)mkl_calloc(rmaxa*cmaxa, sizeof( MKL_INT8 ), 64);
      b = (void *)mkl_calloc(rmaxb*cmaxb, sizeof( MKL_INT8 ), 64);
...
/*       Call GEMM_S8U8S32_PACK_GET_SIZE function to allocate buffer for A (C Interface) */
      identifier = CblasAMatrix;
      size_dest = cblas_gemm_s8u8s32_pack_get_size(identifier, m, n, k);
      dest = (void *)mkl_malloc(size_dest, 64);
      if( dest == NULL ) {
...
/*       Call GEMM_S8U8S32_PACK subroutine to perform packing (C Interface)              */
      cblas_gemm_s8u8s32_pack(layout, identifier, transA, m, n, k, a, lda, dest);

/*       Call GEMM_S8U8S32_COMPUTE subroutine (C Interface)                              */
      storage = CblasPacked;
      cblas_gemm_s8u8s32_compute(layout, storage, transB, offsetc, m, n, k,
