...
/*
!  Content:
!      C B L A S _ C T R S M _ B A T C H  Example Program Text ( C Interface )
!******************************************************************************/
...
int main(int argc, char *argv[])
{
      ...
      MKL_Complex8    alpha[MAX_GRP_COUNT];
      MKL_Complex8    *a, *b;
      MKL_Complex8    **a_array, **b_array;
      CBLAS_LAYOUT    layout;
      ...
      printf("\n     C B L A S _ C T R S M _ B A T C H  EXAMPLE PROGRAM\n");
...
/*       Get input data                                        */
      ...
      for (i=0; i<grp_count; ++i) {
          if( GetScalarsC(in_file, &alpha[i]) != 1 ) {
      ...
      for (l=0; l<total_batch_count; ++l) {
          a_array[l] = (MKL_Complex8 *)NULL;
          b_array[l] = (MKL_Complex8 *)NULL;
      }
      ...
      for (i=0; i<grp_count; ++i) {
          ...
          for (j=0; j<grp_sizes[i]; ++j) {
              ...
              if( GetArrayC(in_file, &layout, typeA, &ma, &na, a, &lda[i]) != 0 ) {
              ...
              if( GetArrayC(in_file, &layout, GENERAL_MATRIX, &m[i], &n[i], b, &ldb[i]) != 0 ) {
...
/*       Print input data                                      */
      ...
      for (i=0; i<grp_count; ++i) {
          ...
          printf("\n       ALPHA=(%5.1f,%5.1f)", alpha[i].real, alpha[i].imag);
          PrintParameters("SIDE, UPLO, TRANSA, DIAG", side[i], uplo[i], transA[i], diag[i]);
          for (j=0; j<grp_sizes[i]; ++j) {
              PrintArrayC(&layout, FULLPRINT, typeA, &ma, &na, a_array[idx], &lda[i], "A");
              PrintArrayC(&layout, FULLPRINT, GENERAL_MATRIX, &m[i], &n[i], b_array[idx], &ldb[i], "B");
              idx++;
          }
      }

/*      Call CTRSM_BATCH subroutine ( C Interface )                  */
      cblas_ctrsm_batch(layout, side, uplo, transA, diag, m, n, alpha,
              (const void **) a_array, lda, (void **) b_array, ldb, grp_count,
              grp_sizes);
