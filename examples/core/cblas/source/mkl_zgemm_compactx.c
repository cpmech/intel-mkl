...
/*
!  Content:
!      M K L _ Z G E M M _ C O M P A C T  Example Program Text ( C Interface )
!******************************************************************************/
...
int main(int argc, char *argv[])
{
  ...
  MKL_Complex16 *a, *b, *c;
  MKL_Complex16 *a_array[MAX_NM], *b_array[MAX_NM], *c_array[MAX_NM];
  double *a_compact, *b_compact, *c_compact;
  ...
  printf("\n     M K L _ Z G E M M _ C O M P A C T  EXAMPLE PROGRAM\n");
...
/*	Set up standard arrays in P2P format			*/
  ...
  c = (MKL_Complex16 *)mkl_malloc( c_total * sizeof(MKL_Complex16), MEM_ALIGNMENT );
  ...
  for (i = 0; i < c_total; i++) c[i].real = rand() / (double) RAND_MAX + .5;
  for (i = 0; i < c_total; i++) c[i].imag = rand() / (double) RAND_MAX + .5;
...
/*       Print input data                                      */
  ...
  for (i = 0; i < nm; i++) {
    ...
    PrintArrayZ(&layout, FULLPRINT, GENERAL_MATRIX, &arows, &acols, a_array[i], &lda, "A");
    PrintArrayZ(&layout, FULLPRINT, GENERAL_MATRIX, &brows, &bcols, b_array[i], &ldb, "B");
    PrintArrayZ(&layout, FULLPRINT, GENERAL_MATRIX, &m,  &n,  c_array[i], &ldc, "C");
  }
...
/*	Set up Compact arrays					*/
  ...
  c_buffer = mkl_zget_size_compact( ldcp, sdcp, COMPACT_FORMAT, nm );

  a_compact = (double *)mkl_malloc( a_buffer, MEM_ALIGNMENT );
  b_compact = (double *)mkl_malloc( b_buffer, MEM_ALIGNMENT );
  c_compact = (double *)mkl_malloc( c_buffer, MEM_ALIGNMENT );

/*	Pack from P2P to Compact format				*/
  mkl_zgepack_compact( (MKL_LAYOUT)layout, arows, acols, (const MKL_Complex16* const*)a_array, lda, a_compact, ldap, COMPACT_FORMAT, nm );
  mkl_zgepack_compact( (MKL_LAYOUT)layout, brows, bcols, (const MKL_Complex16* const*)b_array, ldb, b_compact, ldbp, COMPACT_FORMAT, nm );
  mkl_zgepack_compact( (MKL_LAYOUT)layout, m, n, (const MKL_Complex16* const*)c_array, ldc, c_compact, ldcp, COMPACT_FORMAT, nm );

/*	Perform Compact GEMM					*/
  mkl_zgemm_compact( (MKL_LAYOUT)layout, (MKL_TRANSPOSE)TRANSA, (MKL_TRANSPOSE)TRANSB, m, n, k, &alpha, a_compact, ldap, b_compact, ldbp, &beta, c_compact, ldcp, COMPACT_FORMAT, nm );

/*	Unpack from Compact to P2P format			*/
  mkl_zgeunpack_compact( (MKL_LAYOUT)layout, m, n, c_array, ldc, c_compact, ldcp, COMPACT_FORMAT, nm );
