...
/*
!  Content:
!      M K L _ D G E M M _ C O M P A C T  Example Program Text ( C Interface )
!******************************************************************************/
...
int main(int argc, char *argv[])
{
  ...
  double *a, *b, *c;
  double *a_array[MAX_NM], *b_array[MAX_NM], *c_array[MAX_NM];
  ...
  printf("\n     M K L _ D G E M M _ C O M P A C T  EXAMPLE PROGRAM\n");
...
/*	Set up standard arrays in P2P format			*/
  ...
  c = (double *)mkl_malloc( c_total * sizeof(double), MEM_ALIGNMENT );

  for (i = 0; i < a_total; i++) a[i] = rand() / (double) RAND_MAX + .5;
  for (i = 0; i < b_total; i++) b[i] = rand() / (double) RAND_MAX + .5;
  for (i = 0; i < c_total; i++) c[i] = rand() / (double) RAND_MAX + .5;
...
/*       Print input data                                      */
  ...
  for (i = 0; i < nm; i++) {
    ...
    PrintArrayD(&layout, FULLPRINT, GENERAL_MATRIX, &arows, &acols, a_array[i], &lda, "A");
    PrintArrayD(&layout, FULLPRINT, GENERAL_MATRIX, &brows, &bcols, b_array[i], &ldb, "B");
    PrintArrayD(&layout, FULLPRINT, GENERAL_MATRIX, &m,  &n,  c_array[i], &ldc, "C");
  }
...
/*	Set up Compact arrays					*/
  ...
  c_buffer = mkl_dget_size_compact( ldcp, sdcp, COMPACT_FORMAT, nm );
...
/*	Pack from P2P to Compact format				*/
  mkl_dgepack_compact( (MKL_LAYOUT)layout, arows, acols, (const double* const*)a_array, lda, a_compact, ldap, COMPACT_FORMAT, nm );
  mkl_dgepack_compact( (MKL_LAYOUT)layout, brows, bcols, (const double* const*)b_array, ldb, b_compact, ldbp, COMPACT_FORMAT, nm );
  mkl_dgepack_compact( (MKL_LAYOUT)layout, m, n, (const double* const*)c_array, ldc, c_compact, ldcp, COMPACT_FORMAT, nm );

/*	Perform Compact GEMM					*/
  mkl_dgemm_compact( (MKL_LAYOUT)layout, (MKL_TRANSPOSE)TRANSA, (MKL_TRANSPOSE)TRANSB, m, n, k, alpha, a_compact, ldap, b_compact, ldbp, beta, c_compact, ldcp, COMPACT_FORMAT, nm );

/*	Unpack from Compact to P2P format			*/
  mkl_dgeunpack_compact( (MKL_LAYOUT)layout, m, n, c_array, ldc, c_compact, ldcp, COMPACT_FORMAT, nm );
