...
int main(int argc, char *argv[])
{
      ...
      MKL_INT         ma, na, mb, nb;
      void*           jitter;
      cgemm_jit_kernel_t  kernel;
      mkl_jit_status_t status;

      printf("\n     M K L _ J I T _ C R E A T E _ C G E M M  EXAMPLE PROGRAM\n");
...
/*       Get input data                                        */
      ...
      if( GetArrayC(in_file, (CBLAS_LAYOUT*) &layout, GENERAL_MATRIX, &ma, &na, a, &lda) != 0 ) {
      ...
      if( GetArrayC(in_file, (CBLAS_LAYOUT*) &layout, GENERAL_MATRIX, &mb, &nb, b, &ldb) != 0 ) {
      ...
      if( GetArrayC(in_file, (CBLAS_LAYOUT*) &layout, GENERAL_MATRIX, &m,  &n,  c, &ldc) != 0 ) {
...
/*       Print input data                                      */
      ...
      PrintArrayC((CBLAS_LAYOUT*) &layout, FULLPRINT, GENERAL_MATRIX, &ma, &na, a, &lda, "A");
      PrintArrayC((CBLAS_LAYOUT*) &layout, FULLPRINT, GENERAL_MATRIX, &mb, &nb, b, &ldb, "B");
      PrintArrayC((CBLAS_LAYOUT*) &layout, FULLPRINT, GENERAL_MATRIX, &m,  &n,  c, &ldc, "C");

/*      Call MKL_JIT_CREATE_CGEMM subroutine ( C Interface )                  */

      status = mkl_jit_create_cgemm(&jitter, layout, transA, transB, m, n, k, &alpha,
                                    lda, ldb, &beta, ldc);
      ...
      kernel = mkl_jit_get_cgemm_ptr(jitter);
