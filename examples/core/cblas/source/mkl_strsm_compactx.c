...
/*
!  Content:
!      M K L _ S R T S M _ C O M P A C T  Example Program Text ( C Interface )
!******************************************************************************/
...
int main(int argc, char *argv[])
{
  ...
  float *a, *b;
  float *a_array[MAX_NM], *b_array[MAX_NM];
  ...
  printf("\n     M K L _ S T R S M _ C O M P A C T  EXAMPLE PROGRAM\n");
...
/*	Pack from P2P to Compact format				*/
  mkl_sgepack_compact( (MKL_LAYOUT)layout, adim, adim, (const float* const*)a_array, lda, a_compact, ldap, COMPACT_FORMAT, nm );
  mkl_sgepack_compact( (MKL_LAYOUT)layout, m, n, (const float* const*)b_array, ldb, b_compact, ldbp, COMPACT_FORMAT, nm );

/*	Perform Compact GEMM					*/
  mkl_strsm_compact( (MKL_LAYOUT)layout, (MKL_SIDE)SIDE, (MKL_UPLO)UPLO, (MKL_TRANSPOSE)TRANSA, (MKL_DIAG)DIAG, m, n, alpha, a_compact, ldap, b_compact, ldbp, COMPACT_FORMAT, nm );

/*	Unpack from Compact to P2P format			*/
  mkl_sgeunpack_compact( (MKL_LAYOUT)layout, m, n, b_array, ldb, b_compact, ldbp, COMPACT_FORMAT, nm );
