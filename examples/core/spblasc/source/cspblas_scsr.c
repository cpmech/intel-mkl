...
!          MKL_SCSRSM  MKL_SCSRSV  MKL_SCSRMM  MKL_SCSRMV
!          MKL_CSPBLAS_SCSRGEMV    MKL_CSPBLAS_SCSRSYMV
...
!
!       1. The code computes (L+D)'*S = F using MKL_SCSRMM where S is a known
!          5 by 2 matrix and then the code solves the system (L+D)'*X = F with
!          the help of MKL_SCSRSM. It's evident that X should be equal to S.
!
!       2. The code computes (U+I)'*S = F using MKL_SCSRMV where S is a vector
!          and then the code calls MKL_CSPBLAS_SCSRTRSV solves the system
!          (U+I)'*X = F with the single right hand side. It's evident
!           that X should be equal to S.
!
!       3. The code computes D*S = F using MKL_SCSRMV where S is a vector
!          and then the code solves the system D*X = F with the single
!          right hand side. It's evident that X should be equal to S.
!
!       4. The next step is the computation (U-U') S = F using MKL_SCSRMV where
!          S is a vector. It is easy to see that U-U' is a skew-symmetric matrix.
!
!       5. The next step is the computation (L+D+L') S = F using
!          MKL_CSPBLAS_SCSRSYMV where S is a vector.
...
!
!       6. The next step is the computation A'* S = F using MKL_CSPBLAS_SCSRGEMV
!          where S is a vector.
!
...
!          solves the system T*X =F using MKL_SCSRSV. The routine MKL_SCSRMV
...
//*******************************************************************************
    float      values1[NNZ1] ={1.0, -2.0, 5.0, 4.0,  -4.0, 2.0, 7.0,  8.0, -5.0};
...
//*******************************************************************************
    printf("                             \n");
    printf("   INPUT DATA FOR MKL_SCSRMM \n");
    ...
    mkl_scsrmm(&transa, &m, &n, &m, &alpha, matdescra, values, columns, rowIndex, &(rowIndex[1]), &(sol[0][0]), &n,  &beta, &(rhs[0][0]), &n);
    ...
    mkl_scsrsm(&transa, &m, &n, &alpha, matdescra, values, columns, rowIndex, &(rowIndex[1]), &(rhs[0][0]), &n, &(temp[0][0]), &n);

    printf("                             \n");
    printf("   OUTPUT DATA FOR MKL_SCSRSM\n");
    for (i = 0; i < m; i++) {
        for (j = 0; j < n; j++) {
...
//*******************************************************************************
    ...
    printf("                             \n");
    printf("   INPUT DATA FOR MKL_SCSRMV \n");
    printf("   WITH TRIANGULAR MATRIX    \n");
    printf("     ALPHA = %4.1f  BETA = %4.1f \n", alpha, beta);
    printf("     TRANS = '%c' \n", 'T');
    printf("   Input vector              \n");
    ...
    mkl_scsrmv(&transa, &m, &m, &alpha, matdescra, values, columns, pointerB, pointerE, sol_vec, &beta, rhs_vec);
    ...
    uplo        = 'u';
    nonunit     = 'u';
    mkl_cspblas_scsrtrsv(&uplo, &transa, &nonunit, &m, values, rowIndex, columns, rhs_vec, temp_vec);
...
//*******************************************************************************
    ...
    mkl_scsrsv(&transa, &m, &alpha, matdescra, values, columns, pointerB, pointerE, rhs_vec, temp_vec);
    printf("                             \n");
...
//*******************************************************************************
    ...
    matdescra[2] = 'n';
    matdescra[3] = 'c';
    mkl_scsrmv(&transa, &m, &m, &alpha, matdescra, values, columns, rowIndex, &(rowIndex[1]), sol_vec, &beta, rhs_vec);
    printf("                             \n");
    printf("   OUTPUT DATA FOR MKL_SCSRMV\n");
    printf("   WITH SKEW-SYMMETRIC MATRIX\n");
    for (i = 0; i < m; i++) {
...
//              the help of mkl_cspblas_scsrsymv
// NOTE: The routine mkl_cspblas_scsrsymv as well as the similar dense Level 2
...
//  same output vector
//  mkl_cspblas_scsrsymv(&uplo, &m, values, rowIndex, columns, sol_vec, rhs_vec);
//  mkl_cspblas_scsrsymv(&uplo, &m, values1, rowIndex1, columns1, sol_vec, temp_vec);
//
...
printf("   MKL_CSPBLAS_SCSRSYMV  CALLED TWO TIMES \n");
    for (i = 0; i < m; i++) {
...
//              the help of MKL_CSPBLAS_SCSRGEMV
//*******************************************************************************
    printf("                             \n");
    printf(" MATRIX-VECTOR MULTIPLY ROUTINE FOR GENERAL MATRICES\n");
    printf("   INPUT DATA FOR            \n");
    printf("   MKL_CSPBLAS_SCSRGEMV      \n");
    printf("     TRANS = '%c' \n", 'T');
    printf("   Input vector              \n");
    for (i = 0; i < m; i++) {
        printf("%7.1f\n", sol_vec[i]);
    ...
    mkl_cspblas_scsrgemv(&transa, &m, values, rowIndex, columns, sol_vec, rhs_vec);
    printf("                             \n");
    printf(" MATRIX-VECTOR MULTIPLY ROUTINE FOR GENERAL MATRICES\n");
    printf("   OUTPUT DATA FOR           \n");
    printf("   MKL_CSPBLAS_SCSRGEMV      \n");
...
//    MKL_SCSRMV where S is  3 by 3 minor of the matrix A starting with A(1,1)
...
//*******************************************************************************
    ...
    matdescra[2] = 'n';
    matdescra[3] = 'c';
    mkl_scsrmv(&transa, &mnew, &mnew, &alpha, matdescra, values, columns, pointerB, pointerE, sol_vec, &beta, rhs_vec);
    ...
    mkl_scsrsv(&transa, &mnew, &alpha, matdescra, values, columns, pointerB, pointerE, rhs_vec, temp_vec);
