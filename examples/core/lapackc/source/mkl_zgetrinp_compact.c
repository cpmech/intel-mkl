...
int main() {
    ...
    /* Workspace Query */
    mkl_zgetrinp_compact(layout, n, a_compact, ldap, work_query, lwork, &info, format, nmat);
    lwork = (MKL_INT)work_query[0];
    work = (double *)mkl_malloc(sizeof(double) * lwork, 128);

    /* Perform Compact LU Factorization */
    mkl_zgetrfnp_compact(layout, n, n, a_compact, ldap, &info, format, nmat);

    /* Perform Compact Inverse Calculation */
    mkl_zgetrinp_compact(layout, n, a_compact, ldap, work, lwork, &info, format, nmat);
