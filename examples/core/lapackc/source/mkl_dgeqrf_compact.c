...
int main() {
    ...
    tau_buffer_size = mkl_dget_size_compact(minmn, 1, format, nmat);

    a_compact = (double *)mkl_malloc(a_buffer_size, 128);
    tau_compact = (double *)mkl_malloc(tau_buffer_size, 128);

    /* Pack A matrix from P2P to Compact format */
    mkl_dgepack_compact(layout, m, n, a_array, lda, a_compact, ldap, format, nmat);

    /* Workspace query */
    mkl_dgeqrf_compact(layout, m, n, a_compact, ldap, tau_compact, work_query, lwork, &info, format, nmat);

    lwork = (MKL_INT)work_query[0];
    work = (double *)mkl_malloc(lwork * sizeof(double), 128);

    /* Perform Compact QR Factorization */
    mkl_dgeqrf_compact(layout, m, n, a_compact, ldap, tau_compact, work, lwork, &info, format, nmat);

    /* Unpack A from Compact to P2P format */
    mkl_dgeunpack_compact(layout, m, n, a_array, lda, a_compact, ldap, format, nmat);

    /* Unpack tau from Compact to P2P format */
    mkl_dgeunpack_compact(MKL_COL_MAJOR, minmn, 1, tau_array, minmn, tau_compact, minmn, format, nmat);
