...
int main() {
    ...
    tau_buffer_size = mkl_sget_size_compact(minmn, 1, format, nmat);
    ...
    /* Workspace query */
    mkl_sgeqrf_compact(layout, m, n, a_compact, ldap, tau_compact, work_query, lwork, &info, format, nmat);

    lwork = (MKL_INT)work_query[0];
    work = (float *)mkl_malloc(lwork * sizeof(float), 128);

    /* Perform Compact QR Factorization */
    mkl_sgeqrf_compact(layout, m, n, a_compact, ldap, tau_compact, work, lwork, &info, format, nmat);
    ...
    mkl_sgeunpack_compact(MKL_COL_MAJOR, minmn, 1, tau_array, minmn, tau_compact, minmn, format, nmat);
