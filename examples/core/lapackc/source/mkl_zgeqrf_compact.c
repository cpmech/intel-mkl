...
int main() {
    ...
    tau_buffer_size = mkl_zget_size_compact(minmn, 1, format, nmat);
    ...
    /* Workspace query */
    mkl_zgeqrf_compact(layout, m, n, a_compact, ldap, tau_compact, work_query, lwork, &info, format, nmat);

    lwork = (MKL_INT)work_query[0];
    work = (double *)mkl_malloc(lwork * sizeof(double), 128);

    /* Perform Compact QR Factorization */
    mkl_zgeqrf_compact(layout, m, n, a_compact, ldap, tau_compact, work, lwork, &info, format, nmat);
    ...
    mkl_zgeunpack_compact(MKL_COL_MAJOR, minmn, 1, tau_array, minmn, tau_compact, minmn, format, nmat);
