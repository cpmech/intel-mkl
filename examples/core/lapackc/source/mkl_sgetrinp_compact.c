...
int main() {
    ...
    mkl_sgepack_compact(layout, n, n, a_array, lda, a_compact, ldap, format, nmat);

    /* Workspace Query */
    mkl_sgetrinp_compact(layout, n, a_compact, ldap, work_query, lwork, &info, format, nmat);
    lwork = (MKL_INT)work_query[0];
    work = (float *)mkl_malloc(sizeof(float) * lwork, 128);

    /* Perform Compact LU Factorization */
    mkl_sgetrfnp_compact(layout, n, n, a_compact, ldap, &info, format, nmat);

    /* Perform Compact Inverse Calculation */
    mkl_sgetrinp_compact(layout, n, a_compact, ldap, work, lwork, &info, format, nmat);

    /* Unpack from Compact to P2P format */
    mkl_sgeunpack_compact(layout, n, n, a_array, lda, a_compact, ldap, format, nmat);
