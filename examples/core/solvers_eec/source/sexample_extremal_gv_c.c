...
{   
    ...
    float        Y[3];               /* Y=(X')*X-I */
    float        sparsity;           /* Sparsity of randomly generated matrix */
    MKL_INT      i, j;
    float        smax, t;    
    ...
    status = mkl_sparse_s_create_csr ( &A, SPARSE_INDEX_BASE_ONE, N, N, ia, ia+1, ja, a );

    descrB.type = SPARSE_MATRIX_TYPE_GENERAL; /* Full matrix is stored */    
    status = mkl_sparse_s_create_csr ( &B, SPARSE_INDEX_BASE_ONE, N, N, ib, ib+1, jb, b );
