...
!
!
!  The test calls mkl_sparse_d_svd routine to find several largest singular 
...
int main()
{
    ...
    double   Eig[4] = {1.0, 2.0, 2.2360679, 3.0 }; /* Exact singular values */
    ...
    double  E[5], XL[4*4], XR[5*5]; /* Output: E - k entries of singular values, XL - k corresponding left-singular vectors, XR - k corresponding right-singular vectors */
    double  res[5];                 /* Output: First k components contain the relative residual vector */
    ...
    double      smax;
    double      Y[3*3];              /* Y=(X')*X-I */    

    /* Create handle for matrix A stored in CSR format */
    descr.type = SPARSE_MATRIX_TYPE_GENERAL;
    status = mkl_sparse_d_create_csr ( &A, SPARSE_INDEX_BASE_ONE, N, M, ia, ia+1, ja, a );    
    ...
    printf("mkl_sparse_d_svd output info %d \n",info);
    if ( info != 0 )
    {
        printf("Routine mkl_sparse_d_svd returns code of ERROR: %i", (int)info);
        return 1;
    }
