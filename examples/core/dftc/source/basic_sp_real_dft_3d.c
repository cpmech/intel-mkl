...
static void init_c(MKL_Complex8 *data,
                   int N1, int N2, int N3,
                   int H1, int H2, int H3);
static int verify_r(float *data,
                    int N1, int N2, int N3,
                    int H1, int H2, int H3);
static int verify_c(MKL_Complex8 *data,
                    int N1, int N2, int N3,
                    int H1, int H2, int H3);
...
int main(void)
{
    ...
    data = (float *)mkl_malloc(N3*N2*(N1/2+1)*2*sizeof(float), 64);
    ...
    status = verify_c((MKL_Complex8*)data, N1, N2, N3, H1, H2, H3);
    ...
    init_c((MKL_Complex8*)data, N1, N2, N3, H1, H2, H3);
                    ...
                    int H1, int H2, int H3)
{
     ...
     * input data
     */
    errthr = 2.5f * logf((float) N3*N2*N1) / logf(2.0f) * FLT_EPSILON;
