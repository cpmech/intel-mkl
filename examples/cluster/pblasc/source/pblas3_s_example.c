...
/* Parameters */
const float   zero = 0.0e+0, one = 1.0e+0, two = 2.0e+0, negone = -1.0e+0;
...
/*  Local scalars */
    ...
    float   thresh, diffnorm, anorm, bnorm, residual, eps;

/*  Local arrays */
    float   *A_local, *B_local, *A, *B, *C, *work;
    MKL_INT iwork[ 4 ];
...
/*      Allocate arrays */
        ...
        B_local = (float*) mkl_calloc(n*n, sizeof( float ), 64);
...
/*      Set arrays */
        ...
        for ( j=0; j<n; j++ ){
            for ( i=0; i<n; i++ ){
                if ( j < n-1 ){
                    if ( i <= j ){
                        A_local[ i+n*j ] = one / sqrt( ( float )( (j+1)*(j+2) ) );
                    } else if ( i == j+1 ) {
                        A_local[ i+n*j ] = -one / sqrt( one + one/( float )(j+1) );
                    } else {
                        A_local[ i+n*j ] = zero;
                    }
                } else {
                    A_local[ i+n*(n-1) ] = one / sqrt( ( float )n );
                }
            }
        }
...
/*  Compute precise length of local pieces and allocate array on
    ...
    B = (float*) mkl_calloc(mp*nq, sizeof( float ), 64);
    C = (float*) mkl_calloc(mp*nq, sizeof( float ), 64);
...
/*  Distribute matrices from 0 process over process grid */
    ...
    psgeadd_( &trans, &n, &n, &one, B_local, &i_one, &i_one, descB_local, &zero, B, &i_one, &i_one, descB );
...
/*  Compute norm of A and B */
    ...
    bnorm = pslange_( "I", &n, &n, B, &i_one, &i_one, descB, work );
    if( iam == 0 ){ printf( ".. Norms of A and B are computed ( p?lange ) ..\n" ); }

/*  Compute product C = A*B */
    psgemm_( "N", "N", &n, &n, &n, &one, A, &i_one, &i_one, descA, B, &i_one, &i_one, descB,
