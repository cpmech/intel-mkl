...
/* Parameters */
const double  zero = 0.0E+0, one = 1.0E+0, two = 2.0E+0, oneeighty = 180.0E+0;
...
/*  Local scalars */
    ...
    int     n_int, nb_int;
    double  norm_x, norm_y, rec_norm_x, rec_norm_y, cosine, new_val_i;
    double  length_x, length_y, alpha_deg, alpha_rad, thresh, eps, residual;

/*  Local arrays */
    double  *x_local, *y_local, *x, *y;
    double  swork[ 4 ];
    MKL_INT iwork[ 2 ];
...
/*      Read parameters */
        ...
        fscanf( fin, "%lf alpha, angle between vectors (measured in degrees), 0.0 <= alpha <= 180.0 ", &alpha_deg );
        fscanf( fin, "%lf length of vector x, must be > 0.0 ", &length_x );
        fscanf( fin, "%lf length of vector y, must be > 0.0 ", &length_y );
...
/*      Check if all parameters are correct */
        if( ( n<=0 )||( n>1000 )||( nb<=0 )||( alpha_deg<zero )||( alpha_deg>oneeighty )||( length_x<=zero )||(length_y<=zero) ) {
            ...
            printf( "0.0 <= alpha <= 180.0 - double (degrees)\n" );
            printf( "length_x > 0.0, length_y > 0.0 - double\n" );
...
/*      Pack data into array and send it to other processes */
        ...
        swork[ 2 ] = length_y;
        swork[ 3 ] = thresh;
        dgebs2d_( &ictxt, "All", " ", &i_four, &i_one, swork, &i_four );
        igebs2d_( &ictxt, "All", " ", &i_two, &i_one, iwork, &i_two );
    } else {

/*      Recieve and unpack data */
        dgebr2d_( &ictxt, "All", " ", &i_four, &i_one, swork, &i_four, &i_zero, &i_zero );
...
/*      Allocate arrays for vectors */
        ...
        y_local = (double*) mkl_calloc(n, sizeof( double ), 64);
...
/*  Compute precise length of local pieces and allocate array on
    ...
    x = (double*) mkl_calloc(nq, sizeof( double ), 64);
    y = (double*) mkl_calloc(nq, sizeof( double ), 64);
...
/*  Distribute vectors from 0 process over process grid */
    pdgeadd_( &trans, &n, &i_one, &one, x_local, &i_one, &i_one, descx_local, &zero, x, &i_one, &i_one, descx );
    pdgeadd_( &trans, &n, &i_one, &one, y_local, &i_one, &i_one, descy_local, &zero, y, &i_one, &i_one, descy );
...
/*  ====================================================================
    ...
    pdnrm2_( &n, &norm_x, x, &i_one, &i_one, descx, &i_one );
    pdnrm2_( &n, &norm_y, y, &i_one, &i_one, descy, &i_one );
...
/*  Call PBLAS level 1 routine for multiplication vector by scalar
    ...
    to compute  -----  and  -----
                ||X||       ||Y||                               */
    pdscal_( &n, &rec_norm_x, x, &i_one, &i_one, descx, &i_one );
    pdscal_( &n, &rec_norm_y, y, &i_one, &i_one, descy, &i_one );
...
/*  Call PBLAS level 1 routine to compute scalar product of vectors
    ...
    -----  and  -----
    ||X||       ||Y||                                            */
    pddot_( &n, &cosine, x, &i_one, &i_one, descx, &i_one, y, &i_one, &i_one, descy, &i_one );
...
/*  Print results */
    if ( iam == 0 ){
        printf( "== Results ==\n" );
        printf( "||x||=%03.11f - length of vector x\n", norm_x );
        printf( "||y||=%03.11f - length of vector y\n", norm_y );
        printf( "cos(x^y)=%03.11f\n", cosine );
        printf( "angle(x^y)=%03.11f degrees\n", oneeighty*alpha_rad/M_PI );
