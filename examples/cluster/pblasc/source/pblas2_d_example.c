 ...
 *     PDTRMV,
 *
 *     PDCOPY  (PBLAS Level 1),
 *     PDNRM2  (PBLAS Level 1),
 *     PDLANGE (ScaLAPACK)
 *
...
/*  Local scalars */
    ...
    double  thresh, diffnorm, anorm, residual, eps;

/*  Local arrays */
    double  *A_local, *inv_A_local, *b_local, *A, *inv_A, *x, *b, *work;
    MKL_INT iwork[ 4 ];
...
/*      Allocate arrays */
        ...
        b_local = (double*) mkl_calloc(n, sizeof( double ), 64);
        inv_A_local = (double*) mkl_calloc(n*n, sizeof( double ), 64);

/*      Set arrays */
        for( i=0; i<n; i++ ){
            b_local[ i ] = one*rand()/RAND_MAX;
        }
        dlaset_( "U", &n, &n, &one, &one, A_local, &n );
        dlaset_( "U", &n, &n, &zero, &one, inv_A_local, &n );
        for( i=0; i<n-1; i++ ){
...
/*  Compute precise length of local pieces and allocate array on
    ...
    inv_A = (double*) mkl_calloc(mp*nq, sizeof( double ), 64);

/*  Only #0 process column contains distributed right-hand side vector */
    if ( nq_rhs != 0 ){
        x = (double*) mkl_calloc(mp, sizeof( double ), 64);
        b = (double*) mkl_calloc(mp, sizeof( double ), 64);
    } else {
        x = NULL;
        b = NULL;
    }
...
/*  Distribute vectors and matrices from 0 process over process grid */
    ...
    pdgeadd_( &trans, &n, &n, &one, inv_A_local, &i_one, &i_one, descinvA_local, &zero, inv_A, &i_one, &i_one, descinvA );
    pdgeadd_( &trans, &n, &i_one, &one, b_local, &i_one, &i_one, descb_local, &zero, b, &i_one, &i_one, descb );
...
/*  Copy vector b to x */
    pdcopy_( &n, b, &i_one, &i_one, descb, &i_one, x, &i_one, &i_one, descx, &i_one );


/*  Solve system of equations A*x=b ( on entry array x contains vector b,
    on exit - solution of the system ) */
    pdtrsv_( "U", "N", "U", &n, A, &i_one, &i_one, descA, x, &i_one, &i_one, descx, &i_one );
...
/*  Compute vector inv_A*b ( on entry array b contains vector b,  on exit
    - vector inv_A*b ) */
    pdtrmv_( "U", "N", "U", &n, inv_A, &i_one, &i_one, descA, b, &i_one, &i_one, descb, &i_one );
...
/*  Compute difference  x - inv_A*b  ( it can be done in parallel because
    ...
    pdnrm2_( &n, &diffnorm, x, &i_one, &i_one, descx, &i_one );
...
/*  Compute residual */
    ...
    if( iam == 0 ){
        ...
        printf( "== Results ==\n" );
        printf( "|| x - inv_A*b ||/(2*eps*||A||) = %03.11f\n", residual );
